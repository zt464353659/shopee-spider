...
module.exports = {
  ...
  openIndex: async (params) => {
    ...
    try {
      ...
      const deleteDiscountCoupon = async (params) => {
        let path
        if (params.data.site === 'tw') {
          path = `https://seller.xiapi.shopee.cn/api/marketing/v3/voucher/?SPC_CDS=${ SPC_CDS }&SPC_CDS_VER=${SPC_CDS_VER}`
        } else {
          path = `${ params.platform }/api/marketing/v3/voucher/?SPC_CDS=${ SPC_CDS }&SPC_CDS_VER=${SPC_CDS_VER}`
        }
        return request({
          url: path,
          method: 'delete',
      ...
      return  await deleteDiscountCoupon(params)
    } catch (error) {
      console.log(error)
      /**
